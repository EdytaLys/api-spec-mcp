...
Use the `addCommentToJiraIssue` MCP tool to post a comment on the JIRA story. **CRITICAL**: You MUST set `contentFormat` to `"markdown"` when calling the tool, otherwise the comment will render as raw text.

...
If no spec is found in comments, post a comment saying no approved spec was found and stop.
...
Use `addCommentToJiraIssue` to post the report.

### MANDATORY formatting rules

Use Jira wiki markup. Do NOT set `contentFormat` parameter.

**FORBIDDEN (Markdown) — NEVER use:**
- `#` `##` `###` for headings — use `h1.` `h2.` `h3.` `h4.`
- `| col | col |` with `|---|---|` — use `||col||col||` headers and `|val|val|` rows
- Triple backticks — use `{code}...{code}` or `{noformat}...{noformat}`
- `**bold**` — use `*bold*`
- `*italic*` — use `_italic_`
- `---` — use `----`
...
```
h2. Spec Validation — [ISSUE-KEY]: [Short title]
...
|Warnings|[count]|
|Info|[count]|
|Verdict|{color:green}PASS{color} or {color:red}FAIL{color}|
...
h4. Endpoints Fully Compliant

||Endpoint||Status||
|[METHOD /path]|{color:green}PASS{color}|
|...|...|
...
----
_Generated by spec-validator skill_
```

### If no spec found in comments:

```
h2. Spec Validation — [ISSUE-KEY]: No Approved Spec Found
...
|Verdict|{color:green}PASS{color}|

h4. All Endpoints Compliant

||Endpoint||Params||Request||Response||Errors||Validation||Pagination||Auth||
|[METHOD /path]|{color:green}OK{color}|{color:green}OK{color}|{color:green}OK{color}|{color:green}OK{color}|{color:green}OK{color}|{color:green}OK{color}|{color:green}OK{color}|
